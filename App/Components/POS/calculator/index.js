  ...
  _handleEvent = (value) => {
    ...
    if(!isNaN(value) || value=='.'){
      ...
      switch(value) {
        ...
        default:
          let strLastChar = this.props.value.orderDTO.totalPrice;
  ...
  _concatToOutput = (value) => {
    if(this.state._output.length>=maxLength){
      ...
      if(this.state._output !== initialOutput){
       console.tron.log("xuuux",this.state._output)
        this.setState({_output: this.state._output + '-' + value + ''})
        console.tron.log("xuuux2",this.state._output)
        
      }
      else{
        this.setState({_output: this.props.value.orderDTO.totalPrice + ''})
      }
    }
  }
